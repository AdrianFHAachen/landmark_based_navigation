...
For the simulation only the landmarks that were relevant to reach the given destination were added. This was due to the fact that we could not extract the landmarks directly from Google Earth, since the Gazebo model did not correspond with the real environment. Therefore, extracting the landmarks demanded much more time and computing power than it would in a real-life scenario. More landmarks could be easily added to be able to reach any point on the map.
"""
...
In real life, we would use 6 range sensors. Since there is not a plugin available on Gazebo for that kind of sensors, we have instead used a laser scanner and extracted three readings from the "ranges" matrix. We have only defined the sensor pointing upwards, but the ones pointing downwards would be programmed in exact the same way. The only thing that would need to be changed is that the readings would be taken from the range sensors instead of from a laser scanner.
"""
...
def callback_imu(msg):
	...
	if abs(msg.orientation.x) > 0.35 or abs(msg.orientation.y) > 0.35 or abs(msg.linear_acceleration.x) > 10:
		obstacle = "critical"
